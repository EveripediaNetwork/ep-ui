...
const { hasCopied, onCopy } = useClipboard(`https://alpha.everipedia.org${router.asPath}`)
  ...
  return (
    <Modal onClose={onClose} isOpen={isOpen} isCentered size="xl" {...rest}>
      ...
      >
        ...
        <Flex justify='space-between' align='center' border='2px solid' borderColor='tetiaryGray' p='1rem' borderRadius='0.4rem'> 
            <Text onClick={onCopy} color='brand.600' cursor='pointer'>{hasCopied ? 'Copied!' : 'Copy url'}</Text>
            <Box overflowX='hidden' w='80%' p='0.5rem' bg='brand.150' _dark={{bg: '#232934'}}>
            <Link whiteSpace='nowrap' href={router.asPath} color='#2D3748' _dark={{color: '#b1b2b5'}}>{`https://alpha.everipedia.org${router.asPath}`}</Link>
            </Box>
        </Flex>  

        <Flex fontWeight='bold' mt='3rem' flexDirection='column' > 
            <Text color='#1A202C' fontSize='sm' _dark={{color: "white"}}>Or share via:</Text>
            <Wrap mt='1rem'>
                { 
                    SHARING_OPTIONS.map(item => { 
                        return ( 
                            <item.label url={`https://alpha.everipedia.org${router.asPath}`}>
